...
def process_2nd_floor(file_path):
    ...
    df_final.to_excel(output_path, index=False)
        # Add raw Door Name logs as another sheet in proper order
    raw_data_ordered = df_filtered[["Employee Code", "Employee Name", "Date", "Time", "Door Name."]].copy()
    raw_data_ordered = raw_data_ordered.sort_values(by=["Employee Code", "Date", "Time"])

    with pd.ExcelWriter(output_path, engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
        raw_data_ordered.to_excel(writer, sheet_name="Door Name Inputs", index=False)
...
def process_3rd_floor(file_path):
    ...
    df_selected.to_excel(output_path, index=False)
        # Add raw Punch Records as another sheet in proper order
    raw_data_ordered = df[["Employee Code", "Employee Name", "Date", "Punch Records"]].copy()
    raw_data_ordered["Date"] = pd.to_datetime(raw_data_ordered["Date"]).dt.date
    raw_data_ordered = raw_data_ordered.sort_values(by=["Employee Code", "Date"])

    with pd.ExcelWriter(output_path, engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
        raw_data_ordered.to_excel(writer, sheet_name="Raw Punch Records", index=False)
...
def merged():
    if request.method == "POST":
        ...
        # Save main merged sheet first
        with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
            df_merged.to_excel(writer, index=False, sheet_name="Merged Data")
            try:
                df_2nd_raw = pd.read_excel(file_2nd_path, sheet_name="Door Name Inputs")
                df_2nd_raw.to_excel(writer, sheet_name="2nd floor Inputs", index=False)
            except:
                 pass  # if the sheet doesn't exist, skip it
            try:
                df_3rd_raw = pd.read_excel(file_3rd_path, sheet_name="Raw Punch Records")
                df_3rd_raw.to_excel(writer, sheet_name="3rd floor Inputs", index=False)
            except:
                 pass  # if the sheet doesn't exist, skip it
